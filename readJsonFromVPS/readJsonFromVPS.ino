...
//intialisation du ssid et du password pour le wifi
//const char* ssid = "AttentionChienMechant";
//const char* password = "Twixestnele11avril2022";

//const char* ssid = "WifiLaboSec";
//const char* password = "LaboSec24!";

//const char* ssid = "Remi sans famille";
//const char* password = "remituPuesDuCul1234133221";
...
void setup() {
  ...
  //                   si le fichier contient le bon mdo/ssid, connexion établie -> j'affiche le menu et le réveil démarre normalement
...
void executeMenuOption(int option) {
  switch(option) {
    case 0:
      ...
      isIntoScreen = true;
      SetAlarmScreen();
      // Par exemple, appeler une fonction pour configurer l'alarme
      break;
    case 1:
      // Action pour "Alarm Clock"
      isIntoScreen = true;
      // Par exemple, appeler une fonction pour afficher l'écran de l'horloge
      break;
    case 2:
      isIntoScreen = true;
      // Action pour "Sncb/Alarm"
      //getRequestJson();
      sncbAlarmCLock = true;
...
void manageSetAlarm(){
  ...
  // Programmation terminée, mettre à jour les variables d'alarme
  alarmIsOn = true; 
...
void displayValueAlarm(){
    ...
    display.setTextSize(6);
    display.setCursor(74, 102);
    display.print(String(alarmedHour) + ":" + (alarmMinutes < 10 ? "0" : "") + String(alarmMinutes));
}
...
//redef du constructeur et des méthodes pour la classe Alarm (librairie Alarm)
Alarm :: Alarm()
: playing(false) //bool si l'alarm est en cours de fonctionnement ou pas 
, lastTonTime(0){ //temps de la dernière tonnalité

}

//méthode pour assigner un pin a l'alarm (en mode output car On ou Off)
...
void Alarm::play(){
  if(!playing || lastTonTime + 500 + 100 < millis() ){ //500 pour la durée du ton et 100 pour l'espacement entre les "bip" de l'alarm
    tone(Pin, 500); //utilise la librairie Tone pour avoir une tonnalité différente pour l'alarm (ici 500 mHz)
    lastTonTime = millis(); 
  }
  playing = true;
}
...
void setupRTCModule(){
  ...
  if (rtc.lostPower()) {
    ...
    rtc.adjust(DateTime(2024, 3, 22, 21, 8, 0));
  }
...
void getCurrrentTimeDate(){
  ...
  myTime = String(hour) + ":" + (minutes < 10 ? "0" : "") + String(minutes);
...
void displayTime(){
  ...
  if(needToRefreshClock){
    ...
    //fillRect() X, Y pair for the top-left corner of the rectangle, a width and height (in pixels), and a color.
    display.setRotation(3);
